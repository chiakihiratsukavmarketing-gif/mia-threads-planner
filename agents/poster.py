"""
Threads投稿エージェント
Threads APIを使って安全に投稿する。
- DRY_RUN=true の時は実際に投稿せずログだけ出す
- 1日の投稿上限チェック
- レート制限対応（投稿間隔の確保）
"""
import json
...
RATE_LOG = LOGS_DIR / "rate_limit.json"

MAX_TEXT_LENGTH = 500  # Threads の文字数上限
...
class ThreadsPoster:
    ...
    def post(self, text: str) -> dict:
        ...
        if len(text) > MAX_TEXT_LENGTH:
            return {
                "success": False,
                "post_id": None,
                "error": f"文字数超過: {len(text)}字（上限{MAX_TEXT_LENGTH}字）",
            }
    ...
    def _load_rate_log(self) -> list[dict]:
        if not RATE_LOG.exists():
            return []
        with open(RATE_LOG, encoding="utf-8") as f:
            return json.load(f)

    def wait_between_posts(self) -> None:
        """連続投稿時の待機（自然なリズムを作るため）"""
        if not self.dry_run:
...
def _log_api_error(step: str, resp: requests.Response, extra: dict | None = None) -> None:
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    error_log = LOGS_DIR / "api_errors.json"
    errors = []
    if error_log.exists():
